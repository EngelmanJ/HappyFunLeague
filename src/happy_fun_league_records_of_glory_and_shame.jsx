...
const HEADER_ARTS = [
  { key: "neon", label: "Neon Synthwave", src: "/mnt/data/A_digital_illustration_in_a_neon_retro-futuristic_.png" },
  { key: "inked", label: "Inked Comic", src: "/mnt/data/A_traditional-meets-digital_illustration_in_comic_.png" },
  { key: "gritty", label: "Gritty Painterly", src: "/mnt/data/A_traditional_painting_in_a_realistic_and_comic_bo.png" },
];

export default function HFL_RecordsOfGloryAndShame() {
  ...
  try { return localStorage.getItem("hfl_header_art") || ""; } catch { return ""; }
});
      ...
      <main className="max-w-7xl mx-auto px-4 pb-24 pt-6">
        {!rows.length && (
          <div className="mb-6 rounded-xl border border-slate-800 bg-slate-900 p-4">
            ...
            <p className="text-slate-400 text-sm mt-2">Expect headers like team_id, team_name, owner_first, owner_last, season/year, final standing.</p>
                  ...
                  <table className="min-w-[900px] w-full text-sm">
                    <thead className="bg-slate-900">
                      <tr>
                        ...
                        <th className={ownerColHeaderClass}>Owner</th>
                        ...
                        return (
                          <React.Fragment key={r.team_id}>
                            <tr className={cn(idx % 2 ? "bg-slate-950" : "bg-slate-900/50")}> 
                              <td className="sticky left-0 z-10 bg-slate-950 p-2 border-b border-slate-800 font-mono text-center w-12 min-w-[48px]">{r.team_id}</td>
                              <td className="sticky left-12 z-10 bg-slate-950 p-2 border-b border-slate-800 w-40 min-w-[160px] truncate" title={r.current_name}>{r.current_name}</td>
                              <td className={ownerColCellClass} title={r.current_owner}>{r.current_owner}</td>
                              ...
                              {years.map((y) => {
                                ...
                                const display = Number.isFinite(v) ? lastRender(v, y, podiumOnly) : "";
                                return (
                            ...
                            {showHistory &&
                              segments.map((seg, sidx) => (
                                <tr key={`${r.team_id}_seg_${sidx}`} className={cn(idx % 2 ? "bg-slate-950" : "bg-slate-900/50")}> 
                                  ...
                                  <td className={cn(ownerColCellClass, "text-slate-300")}>{seg.owner} <span className="text-slate-500">({seg.start}-{seg.end})</span></td>
                                  ...
                                  {years.map((y) => {
                                    ...
                                    const podiumColor = v === 1 ? "text-yellow-300" : v === 2 ? "text-slate-200" : v === 3 ? "text-amber-600" : "";
                                    const color = podium ? podiumColor : isLast ? "text-rose-400" : "text-slate-400";
                                    const display = active ? lastRender(v, y, podiumOnly) : "";
                                    return (
